...
class SocketManager:
        ...
        if self.workDetector.isWorkDetected(workResults):
            ...
            if self.workDetector.isLadderDetected(detectedClasses):
                ...
                if self.workDetector.isWorkerDetected(detectedClasses):
                    ...
                    if not self.workDetector.isHelmetDetected(helmetResults):
                        temp.append(Equip.안전모)
            
                    ladderIdx = detectedClasses.index(Label.사다리)
                    ...
                    if self.workDetector.isLadderWorkViolation(ladderPolygon, workerMasks):
                        temp.append(Equip.최상단밑단)
                if len(temp) > 0:
                    self.event.setdefault(EventType.사다리위반, {})
                    for each in temp:
                        self.event[EventType.사다리위반][each] = self.event[EventType.사다리위반].get(each, 0) + 1
            ...
            if self.workDetector.isWeldingDetected(masks, detectedClasses, width):
                ...
                if not self.workDetector.isWeldingmaskDetected(detectedClasses):
                    temp.append(Equip.용접가면)
        
                # Fire distinguisher detected
                if not self.workDetector.isFireExtinguisherDetected(detectedClasses):
                    temp.append(Equip.소화기)
                    
                if len(temp) > 0:
                    self.event.setdefault(EventType.용접위반, {})
                    for each in temp:
                        self.event[EventType.용접위반][each] = self.event[EventType.용접위반].get(each, 0) + 1
            # Cutting detected 
            if self.workDetector.isCuttingDetected(masks, detectedClasses, width):
                ...
                if not self.workDetector.isHelmetDetected(helmetResults):
                    temp.append(Equip.안전모)
                if len(temp) > 0:
                    self.event.setdefault(EventType.절삭위반, {})
                    
                    for each in temp:
                        self.event[EventType.절삭위반][each] = self.event[EventType.절삭위반].get(each, 0) + 1
            # Worker detected
            if self.workDetector.isWorkerDetected(detectedClasses) and isOtherWorkDetected == False:
                ...
                if not self.workDetector.isHelmetDetected(helmetResults):
                    temp.append(Equip.안전모)
                if len(temp) > 0:
                    self.event.setdefault(EventType.장비위반, {})
                    for each in temp:
                        self.event[EventType.장비위반][each] = self.event[EventType.장비위반].get(each, 0) + 1
    ...
    def processEvent(self, eventDic):
        ...
        if isUpdate:
            ...
            self.sendDetectCommand(Command.DETECT, RobotID.ROBOT1, event=joinedEvent)
            
    def processAccident(self, accidentSet):
        for each in accidentSet:
            if each not in self.detectedAccident:
                self.sendDetectCommand(Command.DETECT, RobotID.ROBOT1, EventType.사고, each)
        for each in self.detectedAccident:
            if each not in accidentSet:
                self.sendDetectCommand(Command.NOT_DETECT, RobotID.ROBOT1, EventType.사고, each)
